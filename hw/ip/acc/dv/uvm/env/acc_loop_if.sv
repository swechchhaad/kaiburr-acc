...
//
// Bound into the acc_loop_controller and used to help collect loop information for coverage.

`include "prim_assert.sv"

interface acc_loop_if #(
  localparam int LoopStackIdxWidth = prim_util_pkg::vbits(acc_pkg::LoopStackDepth)
) (
  input              clk_i,
  input              rst_ni,

  // Signal names from the acc_loop_controller module (where we are bound)
  ...
  localparam int ImemAddrWidth = prim_util_pkg::vbits(acc_reg_pkg::ACC_IMEM_SIZE);

  function automatic acc_env_pkg::stack_fullness_e get_fullness();
    if (loop_stack_full) begin
      return acc_env_pkg::StackFull;
    end
    if (current_loop_valid) begin
      return acc_env_pkg::StackPartial;
    end
    return acc_env_pkg::StackEmpty;
  endfunction
  ...
  function automatic void control_loop_counters_out_set_assertion(bit enable);
    ...
    if (enable) begin
      $asserton(0, tb.dut.u_acc_core.u_acc_controller.u_acc_loop_controller);
    end else begin
      $assertoff(0, tb.dut.u_acc_core.u_acc_controller.u_acc_loop_controller);
    end
